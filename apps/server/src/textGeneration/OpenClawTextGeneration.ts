...
import { type OpenClawSettings, type ModelSelection } from "@t3tools/contracts";
...
export const makeOpenClawTextGeneration = Effect.fn("makeOpenClawTextGeneration")(function* (
  ...
  const runOpenClawJson = <S extends Schema.Top>({
    operation,
    cwd,
    prompt,
    outputSchemaJson,
    ...
    Effect.gen(function* () {
      ...
      const promptResult = yield* Effect.gen(function* () {
        ...
        Effect.timeoutOption(OPENCLAW_TIMEOUT_MS),
        Effect.flatMap(
          Option.match({
            onNone: () =>
              Effect.fail(
                new TextGenerationError({ operation, detail: "OpenClaw ACP request timed out." }),
              ),
        ...
        Effect.mapError((cause: EffectAcpErrors.AcpError | TextGenerationError) =>
          isTextGenerationError(cause)
            ...
            : new TextGenerationError({
                operation,
                detail: "OpenClaw ACP request failed.",
                cause,
              }),
        ),
      );
        ...
        return yield* new TextGenerationError({
          ...
          detail:
            promptResult.stopReason === "cancelled"
              ? "OpenClaw ACP request was cancelled."
        ...
        Effect.catchTags({
          SchemaError: (cause) =>
            Effect.fail(
              new TextGenerationError({
                operation,
                detail: "OpenClaw Agent returned invalid structured output.",
                cause,
              }),
            ),
        }),
      ...
      Effect.mapError((cause) =>
        isTextGenerationError(cause)
          ...
          : new TextGenerationError({
              operation,
              detail: "OpenClaw ACP text generation failed.",
              cause,
            }),
      ),
      Effect.scoped,
