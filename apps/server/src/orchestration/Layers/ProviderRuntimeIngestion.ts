...
const make = Effect.gen(function* () {
  ...
  // `mode` is "turn" or "paragraph"; both buffer assistant text.
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      if (reasoningDelta && reasoningDelta.delta.length > 0 && reasoningTurnId) {
        ...
        // "token" is normalized to "paragraph" at the settings layer, so this
        // only guards stale data; the cast keeps the check without lying to tsc.
        const reasoningMode = (streamingMode as string) === "token" ? "paragraph" : streamingMode;
