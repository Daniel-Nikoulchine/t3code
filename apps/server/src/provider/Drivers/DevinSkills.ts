 ...
 *
 * Devin discovers Agent Skills from project and user roots (`.devin/skills`,
 * `.windsurf/skills`, `.agents/skills`, `~/.config/devin/skills`, …) but its
 * ACP command catalog only appears after opening a real session. Scanning the
 * same roots avoids starting an agent and its MCP servers just to populate a
 * composer menu.
 *
...
const discoverSkillsInRoot = Effect.fn("discoverDevinSkillsInRoot")(function* (input: {
  ...
  const visit = Effect.fn("visitDevinSkillDirectory")(function* (
    ...
    if (skillInfo?.type === "File") {
      let frontmatter: DevinSkillFrontmatter | undefined = {};
