 ...
 *
 * Kilo (an OpenCode fork) discovers skills from project roots
 * (`.kilo/skills`, `.agents/skills`) and the user-global
 * `~/.config/kilo/skills` directory. Scanning those roots directly avoids
 * starting an agent (and its MCP servers) just to populate a composer menu.
 *
...
function parseSkillFrontmatter(contents: string): KiloSkillFrontmatter | undefined {
...
const discoverSkillsInRoot = Effect.fn("discoverKiloSkillsInRoot")(function* (input: {
  readonly directory: string;
  readonly scope: "user" | "project";
  readonly budget: KiloSkillScanBudget;
  ...
  const visit = Effect.fn("visitKiloSkillDirectory")(function* (
    directory: string,
    depth: number,
    ...
    if (skillInfo?.type === "File") {
      let frontmatter: KiloSkillFrontmatter | undefined = {};
      ...
      // Kilo requires `name:` in SKILL.md; fall back to the directory name
...
const inspectKiloSkills = Effect.fn("inspectKiloSkills")(function* (
  ...
  const budget: KiloSkillScanBudget = {
