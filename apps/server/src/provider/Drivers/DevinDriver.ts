 ...
 *
 * Devin exposes an ACP server (`devin acp`). Model catalog and capability
 * refreshes happen during the managed provider status check via ACP
 * `initialize` plus `devin models list --format json`.
 *
  ...
  create: ({
    ...
    Effect.gen(function* () {
      ...
      const snapshot = yield* makeManagedDriverSnapshot({
        driverKind: DRIVER_KIND,
        instanceId,
        displayLabel: "Devin snapshot",
        effectiveConfig,
        serverSettings,
        resolveMaintenance,
        buildInitialSnapshot: (provider) =>
          buildInitialDevinProviderSnapshot(provider).pipe(Effect.map(stampIdentity)),
        checkProvider,
        enrichSnapshot: enrichDevinSnapshot,
        httpClient,
      });
