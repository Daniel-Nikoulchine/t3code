...
// ACP does not expose DeepSeek's private `streaming_reasoning` phase. Once it has
...
const resolveCallbackTurnId = (ctx: DeepSeekSessionContext): TurnId | undefined => ctx.activeTurnId;

const resolveSessionCallbackTurnId = (
  sessions: ReadonlyMap<ThreadId, DeepSeekSessionContext>,
  threadId: ThreadId,
): TurnId | undefined => {
...
export function selectDeepSeekPermissionOptionId(
...
function selectAutoApprovedPermissionOption(
  ...
  return (
    selectDeepSeekPermissionOptionId(request, "acceptForSession") ??
    selectDeepSeekPermissionOptionId(request, "accept")
  );
}
...
export function deepseekPromptSettlementBelongsToContext(input: {
  ...
  return Effect.gen(function* () {
    ...
    const beginTurnLiveness = (ctx: DeepSeekSessionContext, turnId: TurnId) =>
      Effect.sync(() => {
        ...
        // DeepSeek's private reasoning phase is not present in the ACP stream.
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = deepseekPromptSettlementBelongsToContext({
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const started = yield* Effect.gen(function* () {
            // dsh exposes no interactive question extension: user-input stays
            // unresolved at the adapter layer (respondToUserInput reports
            // unknown requests). Approvals arrive via session/request_permission.
              ...
              mapAcpCallbackFailure(
                Effect.gen(function* () {
                  ...
                  if (input.runtimeMode === "full-access" || alreadyApproved) {
                    const autoApprovedOptionId =
                      input.runtimeMode === "full-access"
                        ...
                        : selectDeepSeekPermissionOptionId(params, "accept");
                  ...
                  const selectedOptionId =
                    resolved === "cancel"
                      ? undefined
                      : selectDeepSeekPermissionOptionId(params, resolved);
                  if (
          ...
          const requestedStartModelId = deepseekModelSelection?.model
            ? resolveDeepSeekAcpBaseModelId(deepseekModelSelection.model)
            : undefined;
          const currentStartModelId = currentDeepSeekModelIdFromSessionSetup(
            started.sessionSetupResult,
          );
          const currentStartReasoningEffort = currentDeepSeekReasoningEffortFromSessionSetup(
            started.sessionSetupResult,
          );
          ...
          // dsh reports the live selection through session config options (no
          // typed model state); fall back to the setup models when absent.
          const setupSelection = currentDeepSeekSelectionFromConfigOptions(
            yield* acp.getConfigOptions,
          );
          const appliedStartModel = yield* applyDeepSeekAcpModelSelection({
            runtime: acp,
            currentModelId: setupSelection.modelId ?? currentStartModelId,
            currentReasoningEffort: setupSelection.reasoningEffort ?? currentStartReasoningEffort,
          ...
          const boundModelId = appliedStartModel.wiredModelId;
          ...
          const session: ProviderSession = {
            ...
            runtimeMode: input.runtimeMode,
            cwd,
            ...(appliedStartModel.displayModelId
              ? { model: resolveDeepSeekAcpBaseModelId(appliedStartModel.displayModelId) }
              : {}),
            threadId: input.threadId,
            resumeCursor: {
          ...
          const ctx: DeepSeekSessionContext = {
            ...
            currentReasoningEffort:
              requestedStartReasoningEffort !== undefined
                ? normalizeDeepSeekReasoningEffort(requestedStartReasoningEffort)
      ...
      Effect.gen(function* () {
        const prepared = yield* withThreadLock(
          ...
          Effect.gen(function* () {
            ...
            // active turn and cancel the in-flight ACP prompt so DeepSeek takes
            ...
            return yield* Effect.gen(function* () {
              ...
              const requestedTurnModelId = turnModelSelection?.model
                ? resolveDeepSeekAcpBaseModelId(turnModelSelection.model)
                : undefined;
              ...
              // DeepSeek ingests images only. Generic files reach the agent
              ...
              const currentModelId = yield* applyDeepSeekAcpModelSelection({
                runtime: ctx.acp,
              ...
              }).pipe(Effect.map((applied) => applied.wiredModelId));
              ctx.currentModelId = currentModelId;
              if (requestedTurnReasoningEffort !== undefined) {
                ctx.currentReasoningEffort = normalizeDeepSeekReasoningEffort(
                  requestedTurnReasoningEffort,
                );
              }
              const displayModel = displayDeepSeekModelSlug(currentModelId);
              const runtimeInstructions = buildRuntimeInstructions({
                harness: "DeepSeek",
                model: displayModel,
                reasoningEffort: normalizeDeepSeekReasoningEffort(requestedTurnReasoningEffort),
              });
