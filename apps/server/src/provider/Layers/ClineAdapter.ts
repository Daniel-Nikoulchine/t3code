...
// ACP has no per-turn progress deadline; without one a stalled Cline turn
...
const DEFAULT_CLINE_TURN_TIMEOUT_MS = 30 * 60 * 1_000;
...
interface ClineSessionContext {
  ...
  clineSkillNames: ReadonlySet<string> | undefined;
   ...
   * `session/set_model`. Re-sending the same id re-runs Cline's provider
...
function applyRequestedSessionConfiguration<E>(input: {
  ...
  return Effect.gen(function* () {
    ...
    // Mirror the spawn-time `--auto-approve` flag into the live session so a
    // mid-session `runtimeMode` change takes effect without respawning. The
    // toggle is best-effort (see ClineAcpSupport): T3's permission handler
    // already enforces the mode even when the option write is skipped.
    yield* applyClineAcpAutoApprove({
      runtime: input.runtime,
...
function permissionOutcome(
  ...
  const optionId = selectClinePermissionOptionId(request, decision);
  return optionId
...
export function makeClineAdapter(clineSettings: ClineSettings, options?: ClineAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const startSession: ClineAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          // Resolve the ClineSettings used to spawn the ACP child. Production
          ...
          // `updateSettings({ providers: { cline: { binaryPath } } })` calls
            ...
            Effect.mapError(
              (cause) =>
                new ProviderAdapterProcessError({
                  provider: PROVIDER,
                  threadId: input.threadId,
                  detail: "Failed to start the Cline ACP session process.",
                  cause,
                }),
            ),
          );
              ...
              mapExtensionFailure(
                Effect.gen(function* () {
                  ...
                  if (input.runtimeMode === "full-access") {
                    const autoApprovedOptionId = selectClineAutoApprovedPermissionOption(params);
          ...
          yield* applyRequestedSessionConfiguration({
            runtime: acp,
            runtimeMode: input.runtimeMode,
            modelSelection: clineModelSelection,
            mapError: ({ cause, method }) =>
              mapAcpToAdapterError(PROVIDER, input.threadId, method, cause),
          });

          const now = yield* nowIso;
          const configuredModel = resolveClineModelId(clineModelSelection?.model);
          const currentModel =
      ...
      Effect.gen(function* () {
        ...
        const run = Effect.gen(function* () {
          ...
          if (
            ...
            // Cline can redirect text while active, but queues rich media as
            ...
            if (ctx.interruptedTurnIds.has(turnId)) {
              ...
              return yield* new ProviderAdapterRequestError({
                ...
                detail: "Cline image steer was interrupted.",
              });
            }
          }
          ...
          if (input.attachments && input.attachments.length > 0) {
            for (const attachment of input.attachments) {
              // Cline ingests images only. Generic files reach the agent
          ...
          if (ctx.interruptedTurnIds.has(turnId)) {
            ...
            return yield* new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "session/prompt",
              detail: "Cline prompt was interrupted during preparation.",
            });
          }
          ...
          const resolvedModel = resolveClineModelId(model) ?? ctx.session.model ?? "default";
          // Cline reruns provider resolution on every `session/set_model`,
          ...
          const promptResult = yield* ctx.acp
            .prompt({
              prompt: [
                ...
                {
                  type: "text",
                  text: buildRuntimeInstructions({ harness: "Cline", model: resolvedModel }),
                },
              ],
            })
            .pipe(
          ...
          if (Option.isNone(promptResult)) {
            ...
            return yield* new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "session/prompt",
              detail: `Cline turn timed out after ${turnTimeoutMs}ms without completing.`,
            });
          }
    ...
    const respondToUserInput: ClineAdapterShape["respondToUserInput"] = (threadId, requestId) =>
      Effect.gen(function* () {
        yield* requireSession(threadId);
        // Cline only sends session/request_permission, never user-input
        // requests, so there is nothing to answer: fail instead of dropping it.
        return yield* new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "session/request_permission",
          detail: `Cline does not emit user-input requests (unknown request: ${requestId}).`,
        });
      });
