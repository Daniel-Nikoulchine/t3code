/**
 * MinimaxAdapterLive — Minimax CLI (`minimax acp`) via ACP.
 *
...
import {
  discoverMinimaxSkills,
  hasMinimaxSkillMention,
  rewriteMinimaxSkillMentions,
} from "../Drivers/MinimaxSkills.ts";
...
import { type MinimaxAdapterShape } from "../Services/MinimaxAdapter.ts";
...
const MINIMAX_RESUME_VERSION = 1 as const;
// ACP has no per-turn progress deadline; without one a stalled Minimax turn
...
const DEFAULT_MINIMAX_TURN_TIMEOUT_MS = 30 * 60 * 1_000;
...
export interface MinimaxAdapterLiveOptions {
   ...
   * Defaults to the legacy built-in instance id (`minimax`).
   */
   ...
   * the `minimaxSettings` captured at construction.
   *
...
interface MinimaxSessionContext {
  ...
  minimaxSkillNames: ReadonlySet<string> | undefined;
  ...
  stopped: boolean;
  /**
   * Model selection already applied to the live ACP session via the
   * negotiated `model` select. Re-sending the same id re-runs Minimax'
   * provider resolution, which can silently reroute an explicit pick;
   * skipping identical re-applies keeps the session on the endpoint the
   * user selected.
   */
  appliedModel: string | undefined;
  /**
   * Last thinking-effort level applied to the live ACP session via the
   * `thinkingEffort` select. Skips redundant re-sends the same way the
   * model id does; reset on model change because a new model advertises
   * its own levels.
   */
  appliedThinkingEffort: string | undefined;
}
...
function parseMinimaxResume(raw: unknown): { sessionId: string } | undefined {
  if (!isRecord(raw)) return undefined;
  if (raw.schemaVersion !== MINIMAX_RESUME_VERSION) return undefined;
  if (typeof raw.sessionId !== "string" || !raw.sessionId.trim()) return undefined;
  return { sessionId: raw.sessionId.trim() };
}

/**
 * Apply the requested model to the live ACP session.
 *
 * Minimax agent modes (`build`, `plan`, …) are session-scoped agent identities,
 * not approval policies, so T3's `RuntimeMode` is intentionally NOT mapped to
 * `session/set_mode` here: Minimax has no ACP permission-mode concept, and
 * sending a Hermes-style mode id would fail the session start with
 * `InvalidModeError`. Approval behavior is enforced purely through the
 * `session/request_permission` handler (auto-approve in `full-access`).
 */
function applyRequestedSessionConfiguration<E>(input: {
  ...
  return Effect.gen(function* () {
    if (input.modelSelection) {
      yield* applyMinimaxAcpModelSelection({
...
export function selectMinimaxAutoApprovedPermissionOption(
  request: EffectAcpSchema.RequestPermissionRequest,
): string | undefined {
  return selectSessionFirstAutoApprovedPermissionOption(request);
}

export function selectMinimaxPermissionOptionId(
  request: EffectAcpSchema.RequestPermissionRequest,
...
function permissionOutcome(
  ...
  const optionId = selectMinimaxPermissionOptionId(request, decision);
  return optionId
  ...
  return Effect.gen(function* () {
    ...
    const turnTimeoutMs =
      typeof requestedTurnTimeoutMs === "number" && Number.isFinite(requestedTurnTimeoutMs)
        ...
        : DEFAULT_MINIMAX_TURN_TIMEOUT_MS;
    ...
    const stamper = yield* makeEventStamper({
      provider: PROVIDER,
      detail: "Failed to generate Minimax runtime identifier.",
    });
    ...
    const mapExtensionFailure = makeExtensionFailureMapper(
      "Failed to process Minimax ACP extension event.",
    );
    ...
    const logNative = (
      ...
      method: string,
      payload: unknown,
      _source: "acp.jsonrpc" | "acp.minimax.extension",
    ) =>
      Effect.gen(function* () {
    ...
    const emitPlanUpdate = (
      ...
      rawPayload: unknown,
      source: "acp.jsonrpc" | "acp.minimax.extension",
      method: string,
    ) =>
      Effect.gen(function* () {
    ...
    const stopSessionInternal = (ctx: MinimaxSessionContext) =>
      stopScaffoldSession({
        ctx,
        sessions,
    ...
    const startSession: MinimaxAdapterShape["startSession"] = (input) =>
      withThreadLock(
        input.threadId,
        Effect.gen(function* () {
          ...
          const resumeSessionId = parseMinimaxResume(input.resumeCursor)?.sessionId;
          ...
          // Resolve the MinimaxSettings used to spawn the ACP child. Production
          ...
          // `updateSettings({ providers: { minimax: { binaryPath } } })` calls
          // actually take effect when the next session spawns.
          const effectiveMinimaxSettings = options?.resolveSettings
          ...
          const acp = yield* makeMinimaxAcpRuntime({
            minimaxSettings: effectiveMinimaxSettings,
            ...
            Effect.mapError(
              (cause) =>
                new ProviderAdapterProcessError({
                  provider: PROVIDER,
                  threadId: input.threadId,
                  detail: "Failed to start the Minimax ACP session process.",
                  cause,
                }),
            ),
          );
              ...
              mapExtensionFailure(
                Effect.gen(function* () {
                  ...
                  if (input.runtimeMode === "full-access") {
                    const autoApprovedOptionId = selectMinimaxAutoApprovedPermissionOption(params);
          ...
          yield* applyRequestedSessionConfiguration({
            ...
            modelSelection: minimaxModelSelection,
            mapError: ({ cause, method }) =>
              mapAcpToAdapterError(PROVIDER, input.threadId, method, cause),
          });

          const now = yield* nowIso;
          const configuredModel = resolveMinimaxModelId(minimaxModelSelection?.model);
          const currentModel = currentMinimaxModelFromSessionSetup(started.sessionSetupResult);
          const session: ProviderSession = {
            ...
            runtimeMode: input.runtimeMode,
            cwd,
            model: configuredModel ?? currentModel ?? "auto",
            threadId: input.threadId,
            resumeCursor: {
              schemaVersion: MINIMAX_RESUME_VERSION,
          ...
          ctx = {
            ...
            // selection compares equal and skips a redundant RPC.
            appliedModel: configuredModel ?? currentModel ?? "auto",
            appliedThinkingEffort: undefined,
          };
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catch((cause) =>
              Effect.logError("Failed to process Minimax runtime notification.", { cause }),
            ),
          ...
          yield* offerRuntimeEvent({
            ...
            provider: PROVIDER,
            threadId: input.threadId,
            payload: { state: "ready", reason: "Minimax ACP session ready" },
          });
          yield* offerRuntimeEvent({
            type: "thread.started",
            ...(yield* makeEventStamp()),
    ...
    const sendTurn: MinimaxAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const run = Effect.gen(function* () {
          ...
          if (
            ...
            // Minimax can redirect text while active, but queues rich media as
            ...
            if (ctx.interruptedTurnIds.has(turnId)) {
              ...
              return yield* new ProviderAdapterRequestError({
                ...
                detail: "Minimax image steer was interrupted.",
              });
            }
          }
          ...
          if (rawPrompt) {
            // Minimax invokes skills natively as `/name`; the composer inserts
            // `$name`. Rewrite known mentions so the agent sees its own form.
            let minimaxSkillNames = ctx.minimaxSkillNames;
            if (hasMinimaxSkillMention(rawPrompt) && minimaxSkillNames === undefined) {
              const skills = yield* discoverMinimaxSkills(
                ctx.session.cwd,
                options?.environment,
              ).pipe(
              ...
              minimaxSkillNames = new Set(
                skills
                  .filter((skill) => skill.enabled && skill.userInvocable !== false)
                  .map((skill) => skill.name),
              );
              ctx.minimaxSkillNames = minimaxSkillNames;
            }
            const prompt = minimaxSkillNames
              ? rewriteMinimaxSkillMentions(rawPrompt, minimaxSkillNames)
              : rawPrompt;
            promptParts.push({ type: "text", text: prompt });
          }
          if (input.attachments && input.attachments.length > 0) {
            for (const attachment of input.attachments) {
              // Minimax ingests images only. Generic files reach the agent
          ...
          if (ctx.interruptedTurnIds.has(turnId)) {
            ...
            return yield* new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "session/prompt",
              detail: "Minimax prompt was interrupted during preparation.",
            });
          }
          ...
          const resolvedModel = resolveMinimaxModelId(model) ?? ctx.session.model ?? "default";
          // Only send the `model` select when the selection actually
          // changed, so an unchanged per-turn selection cannot disturb the
          // session's already-applied model.
          if (ctx.appliedModel !== resolvedModel) {
            ...
            // A new model advertises its own effort levels; the old choice
            // must not leak across. The runtime tracks the fresh config
            // options after set_model — read them live for the next effort
            // send instead of the stale discovery snapshot.
            ctx.appliedThinkingEffort = undefined;
          }
          // Thinking effort rides the `thinkingEffort` select mcode
          // advertises per thinking-capable model (validated live: effort
          // `high` accepted, turn answered). The picker's choice goes back
          // verbatim; the runtime merges it into the turn payload
          // (`reasoning_effort` on openai-completions). Unknown values are
          // rejected by mcode per model, so a rejection is ignored and the
          // turn proceeds without effort instead of failing.
          const requestedThinking = (
            getModelSelectionStringOptionValue(turnModelSelection, "reasoningEffort") ?? ""
          ).trim();
          // Only send when the select is actually live and the value
          // changed: the picker only offers choices mcode advertised, and
          // the select only exists for thinking-capable variants.
          if (requestedThinking && requestedThinking !== ctx.appliedThinkingEffort) {
            const liveOptions = yield* ctx.acp.getConfigOptions.pipe(
              Effect.orElseSucceed(() => []),
            );
            const liveEffort = minimaxThinkingEffortFromSessionSetup({
              configOptions: liveOptions,
            } as never);
            if (liveEffort?.options.some((option) => option.value === requestedThinking) === true) {
              yield* ctx.acp
                .setConfigOption(MINIMAX_THINKING_EFFORT_CONFIG_ID, requestedThinking)
                .pipe(Effect.ignore);
              ctx.appliedThinkingEffort = requestedThinking;
            }
          }
          if (steeringTurnId === undefined) {
          ...
          const promptResult = yield* ctx.acp
            .prompt({
              prompt: [
                ...
                {
                  type: "text",
                  text: buildRuntimeInstructions({ harness: "Minimax", model: resolvedModel }),
                },
              ],
            })
            .pipe(
          ...
          if (Option.isNone(promptResult)) {
            ...
            return yield* new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "session/prompt",
              detail: `Minimax turn timed out after ${turnTimeoutMs}ms without completing.`,
            });
          }
    ...
    const interruptTurn: MinimaxAdapterShape["interruptTurn"] = (threadId) =>
      withThreadLock(
        threadId,
        Effect.gen(function* () {
    ...
    const respondToRequest: MinimaxAdapterShape["respondToRequest"] = (
      threadId,
      requestId,
      decision,
    ) =>
    ...
    const respondToUserInput: MinimaxAdapterShape["respondToUserInput"] = (threadId, requestId) =>
      Effect.gen(function* () {
        yield* requireSession(threadId);
        // Minimax only sends session/request_permission, never user-input
        // requests, so there is nothing to answer: fail instead of dropping it.
        return yield* new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "session/request_permission",
          detail: `Minimax does not emit user-input requests (unknown request: ${requestId}).`,
        });
      });

    const readThread: MinimaxAdapterShape["readThread"] = (threadId) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
        return { threadId, turns: ctx.turns };
      });

    const rollbackThread: MinimaxAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
    ...
    const stopSession: MinimaxAdapterShape["stopSession"] = (threadId) =>
      withThreadLock(
        threadId,
        Effect.gen(function* () {
          const ctx = yield* requireSession(threadId);
    ...
    const listSessions: MinimaxAdapterShape["listSessions"] = () =>
      Effect.sync(() => Array.from(sessions.values(), (c) => ({ ...c.session })));

    const hasSession: MinimaxAdapterShape["hasSession"] = (threadId) =>
      Effect.sync(() => {
        const c = sessions.get(threadId);
        return c !== undefined && !c.stopped;
      });

    const stopAll: MinimaxAdapterShape["stopAll"] = () =>
      Effect.forEach(sessions.values(), stopSessionInternal, { discard: true });

    yield* Effect.addFinalizer(() =>
      Effect.forEach(sessions.values(), stopSessionInternal, { discard: true }).pipe(
        Effect.catch((cause) =>
          Effect.logError("Failed to emit Minimax session shutdown event.", { cause }),
        ),
