...
function parseKiloResume(raw: unknown): { sessionId: string } | undefined {
  ...
  if (raw.schemaVersion !== KILO_RESUME_VERSION) return undefined;
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process Kilo ACP extension event.",
              cause,
            }),
        ),
      );
      ...
      Effect.gen(function* () {
        ...
        const run = Effect.gen(function* () {
          ...
          if (rawPrompt) {
            ...
            if (hasKiloSkillMention(rawPrompt) && kiloSkillNames === undefined) {
            ...
            const prompt = kiloSkillNames
              ? rewriteKiloSkillMentions(rawPrompt, kiloSkillNames)
              : rawPrompt;
