...
import { discoverKiloSkills } from "../Drivers/KiloSkills.ts";
import {
...
// ACP has no per-turn progress deadline; without one a stalled Kilo turn
...
const DEFAULT_KILO_TURN_TIMEOUT_MS = 30 * 60 * 1_000;
...
interface KiloSessionContext {
  ...
  kiloSkillNames: ReadonlySet<string> | undefined;
   ...
   * `session/set_model`. Re-sending the same id re-runs Kilo' provider
...
function parseKiloResume(raw: unknown): { sessionId: string } | undefined {
  return parseAcpResumeCursor(raw, KILO_RESUME_VERSION);
}

/**
 * Apply the requested model to the live ACP session.
 *
 * Kilo agent modes (`build`, `plan`, …) are session-scoped agent identities,
 * not approval policies, so T3's `RuntimeMode` is intentionally NOT mapped to
 * `session/set_mode` here: Kilo has no ACP permission-mode concept, and
...
export function selectKiloAutoApprovedPermissionOption(
  request: EffectAcpSchema.RequestPermissionRequest,
): string | undefined {
  return selectSessionFirstAutoApprovedPermissionOption(request);
}

export function selectKiloPermissionOptionId(
  request: EffectAcpSchema.RequestPermissionRequest,
...
function permissionOutcome(
  ...
  const optionId = selectKiloPermissionOptionId(request, decision);
  return optionId
...
export function makeKiloAdapter(kiloSettings: KiloSettings, options?: KiloAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const mapExtensionFailure = makeExtensionFailureMapper(
      "Failed to process Kilo ACP extension event.",
    );
    ...
    const logNative = (
      ...
      method: string,
      payload: unknown,
      _source: "acp.jsonrpc" | "acp.kilo.extension",
    ) =>
      Effect.gen(function* () {
    ...
    const emitPlanUpdate = (
      ...
      rawPayload: unknown,
      source: "acp.jsonrpc" | "acp.kilo.extension",
      method: string,
    ) =>
      Effect.gen(function* () {
    ...
    const stopSessionInternal = (ctx: KiloSessionContext) =>
      stopScaffoldSession({
        ctx,
        sessions,
    ...
    const startSession: KiloAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          // Resolve the KiloSettings used to spawn the ACP child. Production
          ...
          // `updateSettings({ providers: { kilo: { binaryPath } } })` calls
            ...
            Effect.mapError(
              (cause) =>
                new ProviderAdapterProcessError({
                  provider: PROVIDER,
                  threadId: input.threadId,
                  detail: "Failed to start the Kilo ACP session process.",
                  cause,
                }),
            ),
          );
              ...
              mapExtensionFailure(
                Effect.gen(function* () {
                  ...
                  if (input.runtimeMode === "full-access") {
                    const autoApprovedOptionId = selectKiloAutoApprovedPermissionOption(params);
          ...
          yield* applyRequestedSessionConfiguration({
            ...
            modelSelection: kiloModelSelection,
            mapError: ({ cause, method }) =>
              mapAcpToAdapterError(PROVIDER, input.threadId, method, cause),
          });

          const now = yield* nowIso;
          const configuredModel = resolveKiloModelId(kiloModelSelection?.model);
          const currentModel = currentKiloModelFromSessionSetup(started.sessionSetupResult);
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catch((cause) =>
              Effect.logError("Failed to process Kilo runtime notification.", { cause }),
            ),
    ...
    const sendTurn: KiloAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const run = Effect.gen(function* () {
          ...
          if (
            ...
            // Kilo can redirect text while active, but queues rich media as
            ...
            if (ctx.interruptedTurnIds.has(turnId)) {
              ...
              return yield* new ProviderAdapterRequestError({
                ...
                detail: "Kilo image steer was interrupted.",
              });
            }
          }
          ...
          if (rawPrompt) {
            // Kilo invokes skills natively as `/name`; the composer inserts
            // `$name`. Rewrite known mentions so the agent sees its own form.
            let kiloSkillNames = ctx.kiloSkillNames;
            if (hasSkillMentionForProvider("kilo", rawPrompt) && kiloSkillNames === undefined) {
              const skills = yield* discoverKiloSkills(ctx.session.cwd, options?.environment).pipe(
                Effect.provideService(FileSystem.FileSystem, fileSystem),
                Effect.provideService(Path.Path, path),
              );
              kiloSkillNames = new Set(
                skills
                  .filter((skill) => skill.enabled && skill.userInvocable !== false)
                  .map((skill) => skill.name),
              );
              ctx.kiloSkillNames = kiloSkillNames;
            }
            const prompt = kiloSkillNames
              ? rewriteSkillMentionsForProvider("kilo", rawPrompt, kiloSkillNames)
              : rawPrompt;
            promptParts.push({ type: "text", text: prompt });
          }
          if (input.attachments && input.attachments.length > 0) {
            for (const attachment of input.attachments) {
              // Kilo ingests images only. Generic files reach the agent
          ...
          const resolvedModel = resolveKiloModelId(model) ?? ctx.session.model ?? "auto";
          // Kilo reruns provider resolution on every `session/set_model`,
          ...
          const promptResult = yield* ctx.acp
            .prompt({
              prompt: [
                ...
                {
                  type: "text",
                  text: buildRuntimeInstructions({ harness: "Kilo", model: resolvedModel }),
                },
              ],
            })
            .pipe(
          ...
          if (Option.isNone(promptResult)) {
            ...
            return yield* new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "session/prompt",
              detail: `Kilo turn timed out after ${turnTimeoutMs}ms without completing.`,
            });
          }
    ...
    const respondToUserInput: KiloAdapterShape["respondToUserInput"] = (threadId, requestId) =>
      Effect.gen(function* () {
        yield* requireSession(threadId);
        // Kilo only sends session/request_permission, never user-input
        // requests, so there is nothing to answer: fail instead of dropping it.
        return yield* new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "session/request_permission",
          detail: `Kilo does not emit user-input requests (unknown request: ${requestId}).`,
        });
      });

    const readThread: KiloAdapterShape["readThread"] = (threadId) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
        return { threadId, turns: ctx.turns };
      });

    const rollbackThread: KiloAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
