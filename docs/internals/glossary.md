...
The backend agent runtime that actually performs work. Seven drivers ship built in: Codex, Claude, Cursor, Grok, Hermes, OpenCode, and Antigravity. See [ProviderService.ts][14], [ProviderAdapter.ts][15], and [CodexAdapter.ts][17] as a representative adapter.
